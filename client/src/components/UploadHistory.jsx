  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-white via-gray-50 to-green-50 text-gray-800">
      ...
      <header className="sticky top-0 z-10 bg-white shadow-md px-6 py-4 flex justify-between items-center border-b border-gray-200">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-indigo-100 text-indigo-700 font-medium rounded-md hover:bg-indigo-200 transition duration-200"
          >
            Dashboard
          </button>
          <button
            onClick={logout}
            className="px-4 py-2 bg-gradient-to-r from-red-100 to-red-200 text-red-600 font-medium rounded-md hover:from-red-200 hover:to-red-300 transition duration-200"
          >
            Logout
          </button>
        </div>
      ...
      <main className="container mx-auto px-6 py-10 space-y-10 flex-grow">
        <h1 className="text-3xl font-bold text-green-800">Upload History</h1>

        {loading ? (
          <div className="text-center text-gray-500 animate-pulse">Loading uploads...</div>
        ) : uploads.length === 0 ? (
          <div className="text-center text-gray-500">No uploads found.</div>
        ) : (
          <div className="overflow-auto max-h-[75vh] rounded-lg shadow border border-gray-200">
            <table className="min-w-full text-sm text-left border-collapse">
              <thead className="bg-gray-100 sticky top-0 z-10">
                <tr>
                  <th className="px-4 py-3 border-b border-gray-300 font-semibold">Filename</th>
                  <th className="px-4 py-3 border-b border-gray-300 font-semibold">Uploaded At</th>
                  <th className="px-4 py-3 border-b border-gray-300 text-center font-semibold">Actions</th>
                </tr>
              </thead>
              <tbody>
                {uploads.map((upload, idx) => (
                  <React.Fragment key={upload._id}>
                    <tr className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      <td
                        className="px-4 py-2 border-b border-gray-200 text-green-700 font-medium hover:underline cursor-pointer"
                        onClick={() => {
                    ...
                    {previewData?._id === upload._id && previewIndex === idx && (
                      <tr className="bg-green-50 animate-fade-in">
                        <td colSpan={3} className="px-6 py-4 border-t border-green-200">
                                ...
                                <thead className="bg-gray-100">
                                  <tr>
                                    {Object.keys(previewData.data[0]).map((header) => (
                                      <th key={header} className="px-3 py-2 border-b border-gray-300 font-medium">
                                        {header}
                                      </th>
                                    ))}
                                  </tr>
